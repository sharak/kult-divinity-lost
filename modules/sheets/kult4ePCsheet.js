...
export default class kult4ePCsheet extends ActorSheet{
  get template(){
    return `systems/kult-divinity-lost/templates/sheets/pc-sheet.hbs`;
  }
  ...
  this.actor.deleteEmbeddedDocuments("Item", [itemId]);
    ...
    html.find('.stability-minus').click(ev =>{
      ...
      if (stability_current < 9){
        ...
        ChatMessage.create({ content: `${this.actor.name} ${game.i18n.localize("kult4e.LostStability")}`, speaker: ChatMessage.getSpeaker({alias: this.name})});
      } else {
        ui.notifications.warn(game.i18n.localize("kult4e.PCIsBroken"))
      }
    });
    ...
    html.find('.stability-plus').click(ev =>{
      ...
      if (stability_current > 0){
        let stability_new = stability_current - 1;
        this.actor.update({'data.stability.value': stability_new});
        ChatMessage.create({ content: `${this.actor.name} ${game.i18n.localize("kult4e.HealedStability")}`, speaker: ChatMessage.getSpeaker({alias: this.name})});
      } else{
        ui.notifications.warn(game.i18n.localize("kult4e.PCIsComposed"))
      }
    });
